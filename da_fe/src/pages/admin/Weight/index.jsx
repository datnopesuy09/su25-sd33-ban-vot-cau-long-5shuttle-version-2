...
function Weight() {
    ...
    // Filter weights based on status and search term
    const filteredWeights = weights.filter((weight) => {
        const matchesStatus = filterStatus === '' || weight.trangThai.toString() === filterStatus;
        const matchesSearch = weight.ten.toLowerCase().includes(searchTerm.toLowerCase());
        return matchesStatus && matchesSearch;
    });

    // Calculate pagination based on filtered data
    const totalPages = Math.ceil(filteredWeights.length / weightsPerPage);
    const startIndex = currentPage * weightsPerPage;
    const currentWeights = filteredWeights.slice(startIndex, startIndex + weightsPerPage);
    ...
    const loadWeights = async () => {
        try {
            ...
            toast.error('Không thể tải danh sách trọng lượng!');
        }
    };
    ...
    const handleDelete = async (id) => {
        try {
            ...
            loadWeights();
            setShowModal(false);
            toast.success('Xóa trọng lượng thành công!');
        } catch (error) {
            console.error('Failed to delete weight', error);
            toast.error('Có lỗi xảy ra khi xóa trọng lượng!');
        }
    };
    ...
    const handleAddweight = async (values) => {
        ...
        try {
            ...
            toast.error('Có lỗi xảy ra khi thêm trọng lượng!');
        }
    };
    ...
    const handleUpdateweight = async (values) => {
        ...
        try {
            ...
            toast.error('Có lỗi xảy ra khi cập nhật trọng lượng!');
        }
    };
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="bg-white shadow-sm border-b border-gray-200 mb-6">
                <div className="px-6 py-4">
                    <h1 className="text-2xl font-bold text-gray-900">Quản lý trọng lượng</h1>
                    <p className="text-gray-600 mt-1">Danh sách và quản lý các trọng lượng trong hệ thống</p>
                </div>
            </div>
                    ...
                    <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
                        <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
                            ...
                            <div className="relative flex-1 max-w-md">
                                ...
                                <input
                                    type="text"
                                    placeholder="Nhập tên trọng lượng để tìm..."
                                    value={searchTerm}
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {currentWeights && currentWeights.length > 0 ? (
                                    currentWeights.map((weight, index) => (
                                        <tr key={weight.id} className="hover:bg-blue-50 transition-colors duration-200">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm font-medium text-gray-900">{weight.ten}</div>
                                            </td>
                                                ...
                                                >
                                                    <span
                                                        className={`w-2 h-2 rounded-full mr-2 ${
                                                            weight.trangThai === 1 ? 'bg-green-400' : 'bg-red-400'
                                                        }`}
                                                    ></span>
                                                    {weight.trangThai === 1 ? 'Đang hoạt động' : 'Ngừng hoạt động'}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-center">
                                    ...
                                    <tr>
                                        <td colSpan="5" className="px-6 py-12 text-center">
                                            <div className="flex flex-col items-center">
                                                ...
                                                <h3 className="text-lg font-medium text-gray-900 mb-1">Không có trọng lượng nào</h3>
                                                <p className="text-gray-500">Hãy thêm trọng lượng đầu tiên để bắt đầu quản lý.</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                        ...
                        <div className="flex items-center justify-between">
                            <div className="text-sm text-gray-700">
                                ...
                                <span className="font-medium">
                                    {Math.min(startIndex + weightsPerPage, filteredWeights.length)}
                                </span>{' '}
                                trong tổng số <span className="font-medium">{filteredWeights.length}</span> trọng lượng
                            </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/10 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4">
                        ...
                        <p className="text-gray-700 mb-8">Bạn có chắc chắn muốn xóa trọng lượng này không?</p>
                ...
                <div className="fixed inset-0 bg-black/10 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4">
                        <div className="flex items-center mb-6">
                            ...
                            <div className="ml-4">
                                <h3 className="text-xl font-bold text-gray-900">Thêm trọng lượng mới</h3>
                                <p className="text-gray-600">Điền thông tin trọng lượng</p>
                            </div>
                        </div>
                        <form onSubmit={handleSubmit(handleAddweight)} className="space-y-6">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    Tên trọng lượng <span className="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 ${
                                        errors.weightName ? 'border-red-500' : 'border-gray-300'
                                    }`}
                                    placeholder="Nhập tên trọng lượng..."
                                    {...register('weightName', { required: true })}
                                />
                                {errors.weightName && (
                                    <p className="mt-1 text-sm text-red-500">Tên trọng lượng là bắt buộc</p>
                                )}
                            </div>
                            <div>
                ...
                <div className="fixed inset-0 bg-black/10 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4">
                        <div className="flex items-center mb-6">
                            ...
                            <div className="ml-4">
                                <h3 className="text-xl font-bold text-gray-900">Cập nhật trọng lượng</h3>
                                <p className="text-gray-600">Chỉnh sửa thông tin trọng lượng</p>
                            </div>
                        </div>
                        <form onSubmit={handleSubmit(handleUpdateweight)} className="space-y-6">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    Tên trọng lượng <span className="text-red-500">*</span>
                                </label>
