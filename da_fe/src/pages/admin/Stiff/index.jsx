...
function Stiff() {
    ...
    // Filter stiffs based on status and search term
    const filteredStiffs = stiffs.filter((stiff) => {
        const matchesStatus = filterStatus === '' || stiff.trangThai.toString() === filterStatus;
        const matchesSearch = stiff.ten.toLowerCase().includes(searchTerm.toLowerCase());
        return matchesStatus && matchesSearch;
    });

    // Calculate pagination based on filtered data
    const totalPages = Math.ceil(filteredStiffs.length / stiffsPerPage);
    const startIndex = currentPage * stiffsPerPage;
    const currentStiffs = filteredStiffs.slice(startIndex, startIndex + stiffsPerPage);
    ...
    const loadStiffs = async () => {
        try {
            ...
            toast.error('Không thể tải danh sách độ cứng!');
        }
    };
    ...
    const handleDelete = async (id) => {
        try {
            ...
            loadStiffs();
            setShowModal(false);
            toast.success('Xóa độ cứng thành công!');
        } catch (error) {
            console.error('Failed to delete stiff', error);
            toast.error('Có lỗi xảy ra khi xóa độ cứng!');
        }
    };
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="bg-white shadow-sm border-b border-gray-200 mb-6">
                <div className="px-6 py-4">
                    <h1 className="text-2xl font-bold text-gray-900">Quản lý độ cứng</h1>
                    <p className="text-gray-600 mt-1">Danh sách và quản lý các độ cứng trong hệ thống</p>
                </div>
            </div>
                    ...
                    <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
                        <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
                            ...
                            <div className="relative flex-1 max-w-md">
                                ...
                                <input
                                    type="text"
                                    placeholder="Nhập tên độ cứng để tìm..."
                                    value={searchTerm}
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {currentStiffs && currentStiffs.length > 0 ? (
                                    currentStiffs.map((stiff, index) => (
                                        <tr key={stiff.id} className="hover:bg-blue-50 transition-colors duration-200">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm font-medium text-gray-900">{stiff.ten}</div>
                                            </td>
                                                ...
                                                >
                                                    <span
                                                        className={`w-2 h-2 rounded-full mr-2 ${
                                                            stiff.trangThai === 1 ? 'bg-green-400' : 'bg-red-400'
                                                        }`}
                                                    ></span>
                                                    {stiff.trangThai === 1 ? 'Đang hoạt động' : 'Ngừng hoạt động'}
                                    </span>
                                </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-center">
                                    ...
                                    <tr>
                                        <td colSpan="5" className="px-6 py-12 text-center">
                                            <div className="flex flex-col items-center">
                                                ...
                                                <h3 className="text-lg font-medium text-gray-900 mb-1">Không có độ cứng nào</h3>
                                                <p className="text-gray-500">Hãy thêm độ cứng đầu tiên để bắt đầu quản lý.</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                        ...
                        <div className="flex items-center justify-between">
                            <div className="text-sm text-gray-700">
                                ...
                                <span className="font-medium">
                                    {Math.min(startIndex + stiffsPerPage, filteredStiffs.length)}
                                </span>{' '}
                                trong tổng số <span className="font-medium">{filteredStiffs.length}</span> độ cứng
                            </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/10 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4">
                        ...
                        <p className="text-gray-700 mb-8">Bạn có chắc chắn muốn xóa độ cứng này không?</p>
                ...
                <div className="fixed inset-0 bg-black/10 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4">
                        <div className="flex items-center mb-6">
                            ...
                            <div className="ml-4">
                                <h3 className="text-xl font-bold text-gray-900">Thêm độ cứng mới</h3>
                                <p className="text-gray-600">Điền thông tin độ cứng</p>
                            </div>
                        </div>
                        <form onSubmit={handleSubmit(handleAddstiff)} className="space-y-6">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    Tên độ cứng <span className="text-red-500">*</span>
                                </label>
                                    <input
                                        type="text"
                                    className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 ${
                                        errors.stiffName ? 'border-red-500' : 'border-gray-300'
                                        }`}
                                    placeholder="Nhập tên độ cứng..."
                                        {...register('stiffName', { required: true })}
                                    />
                                {errors.stiffName && (
                                    <p className="mt-1 text-sm text-red-500">Tên độ cứng là bắt buộc</p>
                                )}
                                </div>
                            <div>
                ...
                <div className="fixed inset-0 bg-black/10 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4">
                        <div className="flex items-center mb-6">
                            ...
                            <div className="ml-4">
                                <h3 className="text-xl font-bold text-gray-900">Cập nhật độ cứng</h3>
                                <p className="text-gray-600">Chỉnh sửa thông tin độ cứng</p>
                            </div>
                        </div>
                        <form onSubmit={handleSubmit(handleUpdatestiff)} className="space-y-6">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    Tên độ cứng <span className="text-red-500">*</span>
                                </label>
